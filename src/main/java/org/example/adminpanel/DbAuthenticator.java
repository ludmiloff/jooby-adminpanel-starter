...
import org.example.adminpanel.dao.MyUserProfile;
import org.example.adminpanel.dao.UserDao;
...
public class DbAuthenticator implements Authenticator<UsernamePasswordCredentials> {
    ...
    @Inject
    public DbAuthenticator(final Jdbi db, @Named("password_salt") String salt) {
        this.db = db;
        this.passwordEncoder = new BasicSaltedSha512PasswordEncoder(salt);
    }
    ...
    public void validate(final UsernamePasswordCredentials credentials, final WebContext context) throws HttpAction {
        ...
        try (Handle h = db.open()) {
            final UserDao dao = h.attach(UserDao.class);

            MyUserProfile profile = dao.findByUsername(username);
            //App.log.info("USER DAO: findByUsername");
            if (profile == null) {
